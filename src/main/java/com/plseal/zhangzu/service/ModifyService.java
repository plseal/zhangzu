...
public class ModifyService {
	...
	public List<Zhangzu> query_db_index(String table_id, String year) throws Exception {
		...
		String sql = "SELECT id,z_date,z_name,IFNULL(z_amount,0),z_type,z_io_div,z_remark,IFNULL(z_m_amount,0) as z_m_amount FROM " + table_id +" WHERE z_date like '" + year + "%' order by z_date desc ";
	...
	public List<Zhangzu>  query_db_for_update_html(String table_id, Integer id) throws Exception {
		...
		String sql = "SELECT id,z_date,z_name,IFNULL(z_amount,0),z_type,z_io_div,z_remark,IFNULL(z_m_amount,0) as z_m_amount FROM " + table_id +" WHERE id = " + id + " order by z_date desc ";
